...
三种情况均以退出码 `1` 结束（`-y` 除外）。多输出（`_N` 后缀展开）同样在提交前检查，命中任一残留副本即按上表处理。流式输出（`-`）与网络 URL 输出（`rtmp://`、`srt://` 等）没有本地目标路径，不受覆盖语义约束。`-y`/`-n` 同时透传给 Worker 侧 ffmpeg，因此远端执行与本地上传/下载两端的覆盖判定一致。
