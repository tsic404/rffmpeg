...
// (e.g. /data/media/x://y) is a local path, not a remote URL. The
...
// path component contains "://". The "file" scheme is excluded: ffmpeg's file
// protocol addresses a local path, not a network endpoint — treating "file://"
// as remote would let a direct-mode output like "file:///etc/cron.d/evil"
// bypass the shared-FS allow-list and be written to an arbitrary local path.
