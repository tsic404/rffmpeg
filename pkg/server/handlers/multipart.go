...
//
// The check is a best-effort snapshot, not a capacity reservation: concurrent
// uploads each compare against the same free value and can collectively exceed
// it. That failure is bounded — the actual spool write fails with ENOSPC and
// is classified as 507, so no produced output is silently lost.
