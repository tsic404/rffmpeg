...
// TestIntegration_NoAuthToken tests that a server started without an auth token
// rejects all API requests (fail closed), including requests with no
// Authorization header.
func TestIntegration_NoAuthToken(t *testing.T) {
	// Setup router without auth token, exactly like main.go (middleware is
	// always installed; the empty token makes it reject everything)
	r := chi.NewRouter()
	r.Use(middleware.Logger)
	r.Use(middleware.Recoverer)
	r.Use(Middleware(""))
	...
	t.Run("JobsEndpoint_NoAuthToken_ShouldFail", func(t *testing.T) {
		body := strings.NewReader(`{"input_files":["test.mp4"],"args":["-c:v","libx264"],"output_filename":"out.mp4"}`)
		req := httptest.NewRequest("POST", "/api/v1/jobs", body)
		req.Header.Set("Content-Type", "application/json")
		rec := httptest.NewRecorder()
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Job submission on server without auth token should return 401, got status %d", rec.Code)
		}
	...
	t.Run("JobsEndpoint_NoAuthToken_WithBearer_ShouldFail", func(t *testing.T) {
		body := strings.NewReader(`{"input_files":["test.mp4"],"args":["-c:v","libx264"],"output_filename":"out.mp4"}`)
		req := httptest.NewRequest("POST", "/api/v1/jobs", body)
		req.Header.Set("Content-Type", "application/json")
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Job submission with bearer token on tokenless server should return 401, got status %d", rec.Code)
		}
	})
}
